...
- (void)loadView {
  ...
  for (int i = 0; i < imageNames.count; i++) {
	...
	if([[UIDevice currentDevice]userInterfaceIdiom]==UIUserInterfaceIdiomPhone) {
				switch ((int)[[UIScreen mainScreen] nativeBounds].size.height) {
						case 1136:
								...
								animationFrame = CGRectMake(self.view.frame.origin.x + 20, self.view.frame.origin.y + 85, 60, 60);
								titleFrame = CGRectMake(self.view.frame.origin.x + 20, self.view.frame.origin.y + 125, 200, 37);
								descriptionFrame = CGRectMake(self.view.frame.origin.x + 20, self.view.frame.origin.y + 165, self.view.frame.size.width - 40, 50);
								break;
						case 1334:
								...
								animationFrame = CGRectMake(self.view.frame.origin.x + 20, self.view.frame.origin.y + 115, 60, 60);
								titleFrame = CGRectMake(self.view.frame.origin.x + 20, self.view.frame.origin.y + 185, 200, 37);
								descriptionFrame = CGRectMake(self.view.frame.origin.x + 20, self.view.frame.origin.y + 225, self.view.frame.size.width - 40, 50);
								break;
						case 2208:
								printf("iPhone 6+/6S+/7+/8+");
	...
	UIImageView *animationImageView = [[UIImageView alloc] initWithFrame:animationFrame];
  ...
  [animationImageView startAnimating];
	...
	UILabel *label_Title = [[UILabel alloc]initWithFrame:titleFrame];
	...
	UILabel *label_Description = [[UILabel alloc]initWithFrame:descriptionFrame];
	...
	//UIButton (button_Thanks) properties
	UIButton *button_Credits = [UIButton buttonWithType:UIButtonTypeCustom];
	[button_Credits addTarget:self action:@selector(process_Thanks) forControlEvents:UIControlEventTouchUpInside];
	[button_Credits setTitle:@"View Credits" forState:UIControlStateNormal];
	[button_Credits setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
	[button_Credits setTitleColor:[UIColor fadedWhiteColor] forState:UIControlStateHighlighted];
	button_Credits.titleLabel.font = [UIFont fontWithName:@".SFUIText-Medium" size:15];
	button_Credits.frame = CGRectMake(self.view.frame.origin.x + 20, self.view.frame.size.height - 80, 100, 45);
	[button_Credits sizeToFit];
	button_Credits.tag = 1989;
	[self.view addSubview:button_Credits];
	[button_Credits release];
	...
	UILongPressGestureRecognizer *longPress = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(process_SafeMode)];
	longPress.minimumPressDuration = 3.0;
	[button_Respring addGestureRecognizer:longPress];
	[longPress release];
	[[self view] addSubview:button_Respring];
	[button_Respring release];
	...
	[label_Version setText:@"Obturo 0.5 (Build 050B) \nDesigned for iOS 11"];
...
- (void)process_SafeMode {
	//posix_spawn method to restart SpringBoard ([system("")] is not available at this point)
	pid_t pid;
	const char* args[] = {"killall", "-ABRT", "SpringBoard", NULL};
	posix_spawn(&pid, "/bootstrap/usr/bin/killall", NULL, NULL, (char* const*)args, NULL);
	//[[objc_getClass("FBSystemService") sharedInstance] exitAndRelaunch:YES];
}
...
- (void)process_Thanks {
	UIViewController *infoViewController = [[UINavigationController alloc] initWithRootViewController:[[PoAppInfoViewController alloc] init]];
